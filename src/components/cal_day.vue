<template>
    <div class="main_window">
        ...
        <simplebar style="width: 100%; height: 100%; outline: none" data-simplebar-auto-hide="false" >

            <div ref="window_cal" style="overflow-x: hidden "  class="calendar_window">
...
export default ({
    ...
    methods: {
        ...
        is_in_day(parsed_time_start, parsed_time_end) {
            let count = 0
        ...
        segregate_column(parsed_time_start, parsed_time_end) {
            let vm = this
            function time_end_to_pro(time_end, time_start){
                return ((time_end - (( time_end - time_end % 100) / 100) * 2) + (time_start*24))/24
            }
            ...
            if (this.is_in_day(parsed_time_start, parsed_time_end) > 1) {
                let tab = this.day_1
        ...
        add_event: function(time_hour_start,time_minute_start, time_hour_end, time_minute_end, title, time_start,time_end, rid, color_id=1, mode=1) {
            ...
            // if (!utlis.is_in_week(day_start, this.time_week) && mode != 5) return

            if (mode == 2) { // co tydzien 
                let days = utilis.get_week(this.time_week)
                day_start = days[day_start.getDay()]
                day_end = days[day_end.getDay()]
            } else if (mode == 3) {// co miesiac
            ...
            } else if (mode == 5) { // dziennie
                day_start.setDate(this.time_week.getDate())
                day_start.setMonth(this.time_week.getMonth())
                day_start.setFullYear(this.time_week.getFullYear())
                day_end.setDate(this.time_week.getDate())
                day_end.setMonth(this.time_week.getMonth())
                day_end.setFullYear(this.time_week.getFullYear())
            }
            ...
            // } 
            // else 
            if (day_start.getTime() < day_end.getTime()) 
            {
                if (day_start.getDate() != this.time_week.getDate() || day_start.getMonth() != this.time_week.getMonth() || day_start.getFullYear() != this.time_week.getFullYear()) {
                    time_hour_start = 0
                    time_minute_start = 0
                }
                if (day_end.getDate() != this.time_week.getDate() || day_end.getMonth() != this.time_week.getMonth() || day_end.getFullYear() != this.time_week.getFullYear()) {
                    time_hour_end = 24
                    time_minute_end = 0
                }
                ...
                this.day_1.push({ID: this.unique ,RID: rid ,text: title, time_start: start/ 24, time_end: end + (  ( end - end % 100) / 100 ) * 2 , colors: color_id   })
                this.unique += 1
                // console.log('segregacha id', this.unique)
                this.segregate_column(start/24, end + (  ( end - end % 100) / 100 ) * 2)
        ...
        to_week(date_time) {
            ...
            function add_day(czas) {
                ...
                vm.$refs.day_head_1.innerHTML =  times[czas.getDay()] + ", " + czas.getDate() + ' ' + month[czas.getMonth()] + ' ' + czas.getFullYear().toString()

            }
            // let times = new Date(date_time)
            this.time_week = new Date(date_time)
            // times.setDate(times.getDate() - (times.getDay() == 0 ? 7 : times.getDay()) + 1)
            add_day(new Date(date_time))
            // for(let i = 0; i < 7; i++) {
        ...
        on_scroll_call() {
            let vm = this
            this.$nextTick(() => {
                vm.$refs.cal_haeder.scrollLeft = vm.$refs.window_cal.scrollLeft
                if (vm.$refs.cal_haeder.scrollLeft > 0) {
                    document.querySelector('.calendar_time_hours').style.position = 'fixed'
                    document.querySelector('.calendar_time_hours').style.backgroundColor = 'white'
                    ...
                    document.querySelector('.calendar_time_hours').scrollTop = vm.$refs.window_cal.scrollTop
                } else {
    ...
    created: function() {
        ...
        this.$root.$refs.calendar_day = this
