            ...
            <div class="pop_view_inputs">
                ...
                <div class="pop_view_input">
                    ...
                    <div class="pop_view_editor_in"> 
                        <div ref="editor_pop_cal" id="main_editor_cal_edit" class="pop_view_editor" ></div>
                    </div>
                </div>
            </div>
...
export default {
        ...
        create_event() {
            ...
            if (this.$refs.title.value != "" && this.editror_rich.root.innerHTML.replaceAll(' ', "") != '') {                
                send.send_post_json(
                    ...
                    this.$root.$refs.app_view.url_path + 'api/calendars/edit.php?id=' + this.cal_id,
                    function(e) {
                        if (e.responseText == "OK") {
                            vm.$root.$refs.main.alert_pop(4, 'Zapisano!' , 'Pomyślnie zaktualizowano kalendarz!')
                            // vm.$root.$refs.main.add_event_to_cal()
                            vm.close()
                        } else {
                            vm.$root.$refs.main.alert_pop(6, 'Błąd!' , 'Nie udało się zaktualizować kalendarza!')
                        }
                    }, function() {
                        vm.$root.$refs.main.alert_pop(6, 'Błąd!' , 'Nie udało się zaktualizować kalendarza!')
            ...
            send.send_get(
                ...
                function(resp) {
                    if (is_json(resp.responseText)) {
                        ...
                        vm.$nextTick(() => {
                            vm.editror_rich.root.innerHTML = item['Cal_desc']
                            vm.$refs.title.value = item['Cal_name']                      
                        })
    ...
    mounted: function() {
        this.editror_rich = new Quill('#main_editor_cal_edit',
            {
                modules: {
                    toolbar: true
                },
        ...
        this.$root.$refs.cal_edit_pop = this
