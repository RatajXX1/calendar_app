        ...
        <div class="pop_view">
            <div class="pop_view_inputs">
                ...
                <div class="text_input">
                    <a>Podaj nową nazwe użytkownika</a>
                    <input ref='email_input' type="text" placeholder="Nazwa użytkownika"/>
                </div>
            </div>
            ...
            if (this.$refs.email_input.value.replaceAll(' ', '') != '') {
                ...
                this.$nextTick(()=> {
                    send.send_post_json(JSON.stringify({
                        'name': vm.$refs.email_input.value
                    }),
                    this.$root.$refs.app_view.url_path + 'api/update_user.php',
                    function(resp) {
                        if (resp.responseText == 'OK') {
                            vm.$root.$refs.main.alert_pop(4, 'Zmieniono nazwe!' , 'Pomyślnie zmieniono nazwe użytkownika!')
                            vm.$refs.email_input.value = ''
                            vm.$root.$refs.user_pop.get_user_data_info()
                            vm.$emit('close_pop')
                        } else {
                            vm.$root.$refs.main.alert_pop(6, 'Błąd!' , 'Nie udało się zmięnić nazwe!')
                        }
                    }, function() {
                        vm.$root.$refs.main.alert_pop(6, 'Błąd!' , 'Nie udało się zmięnić nazwe!')
                    })
