...
if (check_all_param() && enter_to_view()) {
    ...
    if ($mode == 1) {
        ...
        // var_dump($dates);
        $results = $con->query("select ID,title,time_start,time_end,mode,color from events where calendar_id in (Select Cal_ID from Calendars_perm where Selected = 1 and User_id = " . $user_id . ") and ((time_start BETWEEN '" . $dates[0] . "' and '" . $dates[1] . "') or (time_end <= '" . $dates[1] . "' and time_end >= '" . $dates[0] . "' and mode = 6) or (mode > 1 and mode < 6)) limit " . $offset_page . " offset " . $page);
    }
    elseif ($mode == 2)
        $results = $con->query("select ID,title,time_start,time_end,mode,color from events where calendar_id in (Select Cal_ID from Calendars_perm where Selected = 1 and User_id = " . $user_id . ") and ((Date(time_start) = '" . $dates . "' and Date(time_end) = '" . $dates. "') or ((Date(time_start) <= '" . $dates . "' or Date(time_start) = '" . $dates . "') and (Date(time_end) >= '" . $dates . "' or Date(time_end) = '" . $dates . "'  ) and mode = 6) or (mode > 1 and mode < 6)) limit " . $offset_page . " offset " . $page ) ;
